    ...
    unless is_nil(trackers) or trackers == "" do
      dynamicToAdd = trackers |> String.split(",")
               |> Enum.reduce(nil, fn x, acc ->
                      case acc do
                        nil -> dynamic([c], fragment("? in (select id from trackers where name = ?)", c.tracker_id, ^x))
                        acc -> dynamic([c], fragment("? in (select id from trackers where name = ?)", c.tracker_id, ^x) or ^acc)
                      end
                  end)
      case dynamic do
        nil -> dynamicToAdd
